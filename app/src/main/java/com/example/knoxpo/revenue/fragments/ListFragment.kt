...
private const val DEFAULT_VIEW_TYPE = 99999;
...
abstract class ListFragment<T, VH : RecyclerView.ViewHolder> : Fragment() {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        itemListRV?.addOnScrollListener(object : RecyclerView.OnScrollListener() {
            override fun onScrolled(recyclerView: RecyclerView?, dx: Int, dy: Int) {
                ...
                if (dy > 0) {
                    val totalItem = listAdapter?.itemCount
                    ...
                    /*else if (manager is StaggeredGridLayoutManager) {
                        var temp = IntArray(resources.getInteger(R.integer.grid_column_count))
                        temp = manager.findLastVisibleItemPositions(temp)
                        lastVisibleItem = getMax(temp)
                    }*/

                    if (lastVisibleItem + threshold >= totalItem!! && !isFetching)
                        fetchItems()
                }
    ...
    protected fun setSwipe(swipeListener: OnListSwipeListener<T>?) {
        val helper = ItemTouchHelper(object : SimpleCallback(0, ItemTouchHelper.LEFT or ItemTouchHelper.RIGHT) {

            override fun getSwipeDirs(recyclerView: RecyclerView?, viewHolder: RecyclerView.ViewHolder?) = if (canItemSwipe(viewHolder?.adapterPosition)) {
            ...
            override fun onSwiped(viewHolder: RecyclerView.ViewHolder?, direction: Int) {
                ...
                swipeListener?.onSwiped(position, direction, items[position])
            }

        })
        helper.attachToRecyclerView(itemListRV)
    }
